...
// Desktop shader — 40 lines for full quality
...
// Mobile shader — 16 lines for smooth performance on mobile GPUs
...
const Threads: React.FC<ThreadsProps> = ({
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      vertex: vertexShader,
      fragment: isMobile ? fragmentShaderMobile : fragmentShaderDesktop,
      uniforms: {
        iTime: { value: 0 },
        iResolution: {
    ...
    // Lower max render dimension on mobile for better perf
    ...
    const update = (t: number) => {
      ...
      if (FRAME_BUDGET > 0 && t - lastFrame < FRAME_BUDGET) return;
      lastFrame = t;

      const { color, amplitude, distance, enableMouseInteraction: interactive } = propsRef.current;
